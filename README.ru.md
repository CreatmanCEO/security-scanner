...
- [x] **JA3 TLS-фингерпринтинг (Layer 4)** — 97 отпечатков вредоносного ПО из abuse.ch SSLBL. Извлечение JA3 через Suricata + сопоставление через ja3_matcher.py. Детекция по TLS handshake даже на порту 443
- [x] **Безопасная доставка VPN-ключей** — Подписочный URL (рекомендуется) + прямой VLESS URI (запасной) для безопасной раздачи ключей
- [x] **Система рассылки для админов** — FSM-поток: текст, превью с количеством пользователей, подтверждение, отправка всем. Поддержка HTML с fallback при ошибках парсинга. Логирование ошибок отправки
- [x] **Переработка Tone of Voice** — Все пользовательские сообщения упрощены для нетехнических пользователей. Три стиля отчёта: простым языком / с деталями / экспертный
- [x] **Ссылки на приложения** — Inline-кнопка в сообщении скана. Ссылки по ОС: Android (GitHub APK) + iPhone (AppStore) с предупреждениями для РФ. Приложения: Hiddify, v2rayNG, Streisand
- [x] **Сценарий отмены** — Отмена удаляет скан из БД, удаляет VPN-ключ, уведомляет пользователя. «Назад» из приложений не отменяет активный скан
