...
- [Трёхуровневый движок обнаружения](#трёхуровневый-движок-обнаружения)
...
│  │  - Анализирует метаданные соединений (IP, порт, SNI, объёмы) │    │
...
│                 │                                                │
│  ┌──────────────▼──────────────────────────────────────────┐    │
│  │  MalwareDetector — трёхуровневый движок анализа          │    │
│  │                                                          │    │
...
│  │  - Даёт конкретные рекомендации к действию              │    │
...
4. **Весь трафик телефона** проходит через сервер анализа в течение настраиваемого окна (по умолчанию: 5 минут)
...
## Трёхуровневый движок обнаружения
...
│   📋 [Скопировать конфиг] или сканируйте QR     │
│                                                   │
│   ┌────────────┐                                 │
│   │  QR-КОД    │  ← Автосгенерированный VLESS   │
│   └────────────┘                                 │
│                                                   │
...
AI-анализатор подстраивает техническую глубину отчётов под обнаруженный или выбранный уровень экспертизы пользователя. Вот как одна и та же находка — **исходящее SSH-соединение к неизвестному серверу** — представляется на каждом уровне:
...
analysis/
  malware_detector.py     — Трёхуровневый движок обнаружения (порты → поведение → чёрные списки)
  ...
  threat_lookup.py        — Запросы к внешним API (AbuseIPDB, OTX, VirusTotal)
  blacklists/
...
3. **Трёхуровневый анализ** — Сочетание сигнатурного сопоставления (Suricata), поведенческого анализа и корреляции с threat intelligence
...
- [ ] **JA3/JA4 TLS-фингерпринтинг** — Идентификация вредоносного ПО по отпечаткам TLS-хендшейка, даже при использовании стандартного HTTPS (порт 443). Извлечение JA3 через Suricata + база отпечатков вредоносного ПО от abuse.ch
- [ ] **Безопасная доставка VPN-ключей** — Замена открытого VLESS URI на подписочные URL или самоуничтожающиеся одноразовые ссылки для защиты инфраструктуры сервера
